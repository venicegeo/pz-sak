...
describe('Controller: UserController', function () {
    ...
    beforeEach(inject(function ($controller, $rootScope, $injector) {
        ...
        usersRequestHandler = $httpBackend.when(
            'GET',
            '/proxy/' + discover.securityHost + '/user').respond(
            {"statusCode": 200,
                "data": [
                {
                    "name": "testuser",
                    "roles": "admin,user,pzuser"
                },
                {
                    "name": "othertestuser",
                    "roles": "user,pzuser"
        ...
        updateRequestHandler = $httpBackend.when(
            'PUT',
            '/proxy/' + discover.securityHost + '/user/testuser',
            {
                "name": "testuser",
                "roles": "admin,user,pzuser,newrole"
            }).respond(
            {
                "statusCode": 200
            }
    ...
    /*it('should get the first 10 users', function () {
        scope.getLogs(0);
        $httpBackend.expectGET('/proxy/' + discover.securityHost + '/user');
        $httpBackend.flush();
